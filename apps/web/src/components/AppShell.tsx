...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${open ? 'sidebar--open' : ''}`}>
      <div className="sidebar__brand"><BrandLogo /><div><strong>SecAttend</strong><span>Workforce operations</span></div><button className="icon-button sidebar__close" onClick={() => setOpen(false)} aria-label="Close navigation"><X /></button></div>
    ...
    <div className="main-column"><header className="mobile-header"><button className="icon-button" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu /></button><div className="mobile-header__brand"><BrandLogo decorative /><strong>SecAttend</strong></div><Avatar name={user!.name} size="sm" /></header><main>{children}</main></div>
  </div>;
}
