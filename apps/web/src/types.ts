...
export interface Expense { id: string; title: string; category: string; amount: string | number; expenseDate: string; note?: string; createdBy?: { id: string; name: string } | null; createdAt?: string }
export interface DashboardData { stats: { totalGuards: number; activeManagers: number; markedToday: number; presentToday: number; absentToday: number; attendancePercent: number; unmarkedToday: number; monthlyGuardPayroll: number; monthlyCompanyBilling: number }; locationStats: Location[]; recentActivity: { id: string; action: string; entity: string; createdAt: string; actor?: { name: string } }[] }
