...
type Summary = { attendance: { present: number; absent: number; marked: number; activeGuards: number }; payroll: { guardNet: number; guardPaid: number; companyNet: number; margin: number }; expenses: number; profit: number };
...
export function ReportsPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Live insights & governance" title="Reports" description="Attendance, compliance, and payroll metrics calculated directly from current records." actions={<><button className="button button--secondary" onClick={exportSiteRegister} disabled={!locationId}><FileBarChart size={17} />Site register</button><button className="button button--primary" onClick={exportAttendance}><Download size={17} />Export attendance</button></>} />
    ...
    <section className="report-metrics"><article><span>Company billing</span><strong>{currency(summary?.payroll.companyNet ?? 0)}</strong><p>Live invoice value</p></article><article><span>Guard paid</span><strong>{currency(summary?.payroll.guardPaid ?? 0)}</strong><p>Marked paid this month</p></article><article><span>Expenses</span><strong>{currency(summary?.expenses ?? 0)}</strong><p>Recorded company spending</p></article><article><span>Profit</span><strong className={(summary?.profit ?? 0) < 0 ? 'deduction' : undefined}>{currency(summary?.profit ?? 0)}</strong><p>Billing minus guard paid and expenses</p></article></section>
