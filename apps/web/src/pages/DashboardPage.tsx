...
export function DashboardPage() {
  ...
  const cards = user!.role === 'ADMIN' ? [
    ...
    { label: 'Attendance today', value: `${stats.attendancePercent}%`, note: `${stats.markedToday} of ${stats.totalGuards} marked`, icon: UserCheck, tone: 'orange' },
    { label: 'Present today', value: stats.presentToday, note: `${stats.absentToday} absent`, icon: CheckCircle2, tone: 'blue' },
  ];
  return <div className="page page--dashboard">
    <PageHeader eyebrow={`${greeting}, ${user!.role === 'ADMIN' ? 'Admin' : user!.name.split(' ')[0]}`} title="Here’s today’s field picture." description={new Intl.DateTimeFormat('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(new Date())} actions={<Link className="button button--primary" to="/attendance"><CalendarCheck size={18} />Mark attendance</Link>} />
