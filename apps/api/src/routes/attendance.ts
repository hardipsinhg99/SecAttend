...
attendanceRouter.get('/calendar/summary', asyncHandler(async (req, res) => {
  ...
  const summary = new Map<string, { date: string; marked: number; present: number; absent: number; leave: number; total: number; state: string }>();
  for (const row of rows) {
    const key = row.date.toISOString().slice(0, 10);
    const total = guards.filter((guard) => !guard.joiningDate || guard.joiningDate <= row.date).length;
    const item = summary.get(key) ?? { date: key, marked: 0, present: 0, absent: 0, leave: 0, total, state: 'none' };
    item.marked += row._count;
    if (row.status === 'PRESENT') item.present += row._count;
    else if (row.status === 'ABSENT') item.absent += row._count;
...
attendanceRouter.post('/:date', asyncHandler(async (req, res) => {
  ...
  const input = z.object({ records: z.array(z.object({ guardId: z.string(), status: z.enum(['PRESENT', 'ABSENT', 'ON_LEAVE']) })).min(1).max(500) }).parse(req.body);
